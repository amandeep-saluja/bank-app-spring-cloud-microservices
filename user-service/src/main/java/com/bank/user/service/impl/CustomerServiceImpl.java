...
public class CustomerServiceImpl implements CustomerService{
	...
	public Customer createCustomer(Customer customer) {
		...
		ResponseEntity<Account> response = restTemplate.postForEntity(ACCOUNT_SERVICE_URL+"add", entity, Account.class);
		CustomerEntity customerEntity = new CustomerEntity((int) (repository.count() + 101), customer.getName(), customer.getPassword(),
	...
	public Account getAccountById(Integer integer) {
		...
		ResponseEntity<Account> response = restTemplate.postForEntity(ACCOUNT_SERVICE_URL + integer, entity, Account.class);
